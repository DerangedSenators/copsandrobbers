...
namespace DerangedSenators.CopsAndRobbers.GameObjects.Player
{
    ...
    public class PlayerRespawn : MonoBehaviour
    {
        ...
        public void OnTriggerEnter2D(Collider2D collider)
        {
            ...
            if (GetComponent<PlayerHealth>().getIsAlive() == false && collider.gameObject.layer == gameObject.layer &&
                teamAlive)
            {
                countdown = true;
                ShowFloatingText();
        ...
        {
            if (countdownTimer <= 2.5) countdownTimer += 1 * Time.deltaTime;
        }
        ...
        {
            var vector3 = new Vector3(transform.position.x, transform.position.y + 1.5f, transform.position.z - 2f);
